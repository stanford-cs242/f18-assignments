const wat = require("buffer-loader!./hash.wat");
...
const module = Promise.resolve(wat).then(wat => wabt.parseWat("hash.wat", wat));

// Show WASM parsing error.
module.catch(e =>
  $("#output").html(`<div class="alert alert-danger">
    <strong>Error While Parsing hash.wat</strong>
...
validated.catch(e =>
  $("#output").html(`<div class="alert alert-danger">
    <strong>Error while validating hash.wat</strong>
  </div><pre><code>${e}</code></pre>`)
);
