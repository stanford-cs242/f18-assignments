...
describe("Basic Class Tests", function()
  ...
  it("Type Tests", function()
    local p1 = ChildClass:new(1, 2)
    assert.is_true(p1:isinstance(class.Object))
    assert.is_true(p1:isinstance(ParentClass))
    assert.is_true(p1:isinstance(ChildClass))
    assert.is_false(p1:isinstance(GrandchildClass))
    ...
    assert.is_true(p2:isinstance(Class))
    assert.is_true(p2:isinstance(class.Object))
  end)
end)
